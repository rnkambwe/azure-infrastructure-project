...
o	vnet1-nsg: RDP (3389), HTTP (80), SMB (445) for Windows workloads
o	vnet2-nsg: SSH (22) for Linux workloads
...
o	Require a tag and its value on resources: Enforced mandatory tagging (environment: project) for all resources, enabling cost tracking and operational visibility.
o	Allowed locations: Restricted resource creation to approved regions (Australia East, UK South) to ensure compliance with data residency requirements.
...
o	Windows Server 2019 VMs (n01677712-w-vm1, n01677712-w-vm2) in availability set (windows-avs), ensuring fault isolation and 99.95% SLA.
o	Ubuntu Server 20.04 VMs (n01677712-u-vm1, n01677712-u-vm2) in availability set (linux-avs), providing redundancy for Linux workloads.
...
o	Microsoft Antimalware on Windows VMs for threat protection.
o	Network Watcher Agent for Linux on Ubuntu VMs for enhanced network monitoring and diagnostics.
Load Balancing & Application Delivery
•	Deployed a public Azure Load Balancer (n01677712-lb1) with:
o	Frontend IP configuration using a public IP address.
o	Backend pool containing both Windows VMs for traffic distribution.
o	Health probes to monitor VM availability and automatically route traffic away from unhealthy instances.
o	Load balancing rules to distribute HTTP traffic (port 80) across backend VMs.
•	Validated load balancer functionality by:
o	Accessing individual VM public IPs to confirm web server operation.
o	Testing the load balancer public IP to verify traffic distribution.
o	Performing failover tests by stopping individual VMs and confirming traffic redirection to healthy instances.
...
o	Daily backup schedule at 6:00 AM.
o	8-day retention for daily backup points.
o	1-day snapshot retention for rapid recovery.
...
o	Management group hierarchy
o	Subscription and resource group structure
o	Virtual network topology and subnet segmentation
o	VNet peering relationships
o	Availability sets and VM deployment
o	Storage accounts, file shares, and private endpoints
o	Load balancer configuration
o	Recovery Services Vault and backup policies
